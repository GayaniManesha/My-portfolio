...
const Navbar = () => {
  ...
  // Determine if we are on the Home page and not scrolled
  const isHomeAndNotScrolled = location.pathname === '/' && !scrolled;

  return (
    <nav className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-md shadow-md' : 'bg-transparent'}`}>
